...
def train(model, train_dataloader, val_dataloader=None, optimizer=None, scheduler=None, epochs=4, evaluation=False,device=None,loss_fn=None, class_name=[]):
    ...
    for epoch_i in range(epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            output_sig = [F.softmax(l) for l in logits]
            # Compute loss and accumulate the loss values
            loss = 0
            for i in range(len(output_sig)):
                loss += loss_fn(logits[i], b_labels[i])
            loss = loss/len(output_sig)
...
def evaluate(model, val_dataloader,device=None,loss_fn=None,class_name=""):
    ...
    for batch in val_dataloader:
        ...
        # Compute loss
        for i in range(len(output_sig)):
            loss = loss_fn(output_sig[i], b_labels[i])
            val_loss.append(loss.item())

            # Get the predictions
            logits[i][output_sig[i] >= 0.5] = 1
            logits[i][output_sig[i] < 0.5] = 0
...
def main():
    ...
    train_data = TensorDataset(traindf[options.xcol], train_labels1, train_labels2)
    train_sampler = RandomSampler(train_data)
    train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=batch_size)

    # Create the DataLoader for our validation set
    val_data = TensorDataset(validdf[options.xcol], val_labels1, val_labels2)
