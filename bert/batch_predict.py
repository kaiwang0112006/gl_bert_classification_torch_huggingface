...
def get_pred(model, tokenizer, text, device, max_length=128):
    try:
...
def main():
    ...
    model.load_state_dict(
        torch.load("/work/kw/yuqing/gl_bert_classification_torch_huggingface/bert/ernie/model/weight.ckpt",
                   map_location=device))
    tokenizer = AutoTokenizer.from_pretrained("/work/pretrain/huggingface/ernie_pretrain")

    datadf["pred"] = datadf["text"].apply(lambda t: get_pred(model, tokenizer, t, device))
    ...
    resultdf.to_csv("model_batch_pred_%s.csv" % d_before_1, index=False, encoding='gbk',error="ignore")
    '''
    db_info = {
        "host":"115.231.21.21",
        "port":33202,
